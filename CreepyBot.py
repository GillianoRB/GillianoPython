...
@bot.command()
async def help(ctx):
    await bot.say("help is not added yet contact Chillergilly to ask the commands")
...
async def removeticket(ctx):
    Ticketchannel = discord.utils.get(bot.get_all_channels(), server__name='Creepergilly Hangout', name='ticket')
...
async def verify(ctx):
    role = discord.utils.get(ctx.message.server.roles, name="Guests")
